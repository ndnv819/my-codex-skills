...
- Profiling slow React UI, laggy forms, expensive lists, heavy component trees, or frequent re-render paths.
- Reviewing memoization, prop identity, callback stability, hook dependencies, effects, context usage, or list virtualization.
- Validating that a React performance change improves the targeted interaction without changing behavior.

## Core Rules

- Establish the slow interaction, affected component, input size, and expected behavior before changing code.
- Prefer evidence over blanket optimization.
- Do not add `memo`, `useMemo`, or `useCallback` unless prop identity or computation cost actually matters.
- Apply the smallest optimization that removes the measured or clearly reasoned cost.
- Preserve user-visible behavior unless the user explicitly asks for a behavior change.
...
6. Report the cause, change, validation method, and any remaining uncertainty.
...
Move fast state down. Keep high-frequency state close to the UI that actually needs it. A parent containing heavy children should not re-render on every keystroke, tick, hover, resize, or scroll event unless those children depend on that state.

Narrow component props. Pass primitive or narrow props where possible. A memoized child receiving a large object that is rebuilt each render still re-renders.

Stabilize identities deliberately. Use `useCallback` for handlers passed to memoized children or dependency-sensitive hooks. Use `useMemo` for expensive derived values or stable object/array props. Avoid using either only to silence lint warnings.

Split context. Separate frequently changing context values from stable actions/configuration. Memoize provider values and split providers when unrelated consumers are forced to update together.

Virtualize long lists. For large lists, tables, logs, chat histories, search results, or grids, avoid rendering offscreen rows.
...
Defer heavy UI. Lazy-render expensive panels, collapsed sections, previews, charts, Markdown, syntax highlighting, or diff views until visible or expanded.

Keep effects boring. Effects should not create render loops. Stabilize dependencies, move pure derivation into render or `useMemo`, and avoid state updates in effects when the value can be computed directly.

## Tools And References

- Use React DevTools Profiler when a browser is available.
- Load `references/examples.md` when concrete React refactor patterns or snippets are needed.
- Prefer existing project test, typecheck, lint, and browser reproduction commands over adding new tooling.
...
- the slow interaction or render trigger identified
- the optimization applied and why that scope was enough
- validation commands, profiler comparison, or reproduction result
- any profiling limitations or remaining performance risks
